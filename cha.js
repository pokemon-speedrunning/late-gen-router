...
let actions = [
    {type: "init", level: 8, tradeExp: true, curve: "Medium Fast"},
    {type: "kill", pokemon: "Rattata-Alola", level: 11}, // 216
    {type: "kill", pokemon: "Rattata-Alola", level: 11}, // 216
    {type: "kill", pokemon: "Drowzee", level: 11}, // 256
    {type: "kill", pokemon: "Raticate-Alola", level: 11}, // 519
    {type: "affection-boost", value: true},
    {type: "kill", pokemon: "Raticate-Alola", level: 12}, // 674
    {type: "roto-exp", value: true},
    {type: "kill", pokemon: "Furfrou", level: 13}, // 1161
    {type: "kill", pokemon: "Litten", level: 13}, // 394
];
...
function getExperienceForKill(foeName, foeLevel, level, luckyEgg, affection, rotoExp, expShare, tradeExp, evolutionDue) {
    let expYield = experienceYields[foeName] || 0;

    let a = 1;
    let b = expYield;
    let e = luckyEgg ? 1.5 : 1;
    let f = affection ? 1.2 : 1;
    let L = foeLevel;
    let Lp = level;
    let p = rotoExp ? 1.2 : 1;
    let s = expShare ? 2 : 1;
    let t = tradeExp ? 1.5 : 1;
    let v = evolutionDue ? 1.2 : 1;

    let calc = ((((a * b * L) / (5 * s)) * Math.pow(((2 * L) + 10), 2.5) / Math.pow((L + Lp + 10), 2.5)) + 1) * e * f * p * t * v;
...
function processActions(actions) {
    ...
    for (let i = 1; i < actions.length; ++i) {
        ...
        switch (action.type) {
            case "kill":
                let receivedExp = getExperienceForKill(action.pokemon, action.level, currentLevel, luckyEgg, affectionBoost, rotoExp, !!action.expShare, tradeExp, false);
